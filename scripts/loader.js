...
const loadData = () => {
	...
	// Load background images (paths fixed - removed leading /)
	...
	// Load audio files (paths fixed - removed leading /)
	...
	queue.on("error", (event) => {
		// Fallback for missing audio files - create dummy audio objects
		console.warn(`Failed to load: ${event.item.id}`);
		if (event.item.id === "correct" || event.item.id === "wrong" || 
		    event.item.id === "win" || event.item.id === "lose" ||
		    event.item.id === "rotate" || event.item.id === "pullup" || 
		    event.item.id === "hang") {
			// Create dummy audio object that won't throw errors
			let dummyAudio = {
				play: () => console.log(`Audio played: ${event.item.id}`),
				pause: () => {},
				volume: 0.5
			};
			handleLoadComplete(event.item.id, dummyAudio);
		}
	});
