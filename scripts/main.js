...
const getRandomWord = () => {
	if (words.length > 0) {
		...
		// Handle empty words - return default word
...
const updateBestScore = () => {
	if (score > bestScore) {
		...
		// Save best score to localStorage for persistence across sessions
	...
	return fetch(endpoint)
		.then(
			...
			(networkError) => {
				...
				// Return fallback data for offline scenarios
...
const fillData = (jsonResponse, cat) => {
	if (cat === "pokemons") {
		...
		if (jsonResponse.length > 0) {
			...
			// Fallback words for the selected category
			const fallbackWords = {
...
// Load saved best score when game starts
loadSavedBestScore();
